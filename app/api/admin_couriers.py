...
@router.get("/couriers")
def couriers_list(db: SessionDep, _admin: str = Depends(admin_subject)):
    items = list_couriers_admin(db)
    return success(data=[dump_model(i) for i in items], msg="获取成功")


@router.post("/couriers")
def couriers_create(body: CourierCreateIn, db: SessionDep, _admin: str = Depends(admin_subject)):
