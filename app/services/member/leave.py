...
def _member_on_leave_prep_schedule_for_period(
    db: Session,
    member: Member,
    *,
    ...
    """备餐锁窗：仅当目标餐段在履约业务日应配送时才阻断请假变更。"""
    from app.models.enums import MealPeriod
    from app.services.dinner.schedule import member_on_dinner_delivery_schedule
    from app.services.meal_period.leave_scope import LEAVE_MEAL_PERIOD_ALL, normalize_leave_meal_period
    ...
    if scope == MealPeriod.DINNER.value:
        return member_on_dinner_delivery_schedule(
            db, member, delivery_date=delivery_date, today=today
        )
    return member_on_lunch_delivery_schedule(
        db, member, delivery_date=delivery_date, today=today
    )
...
def guard_miniprogram_leave_prep_window(
    ...
    *,
    now: datetime | None = None,
    leave_meal_period: str | None = None,
) -> None:
    """
    备餐锁窗内：小程序禁止请假/取消请假等自助变更。
    leave_meal_period 指定时仅阻断「该餐段在锁窗履约日应配送」的变更；未传则保持现网午餐行为（锁窗内一律禁止）。
    """
    from app.services.shared.store_config_service import get_leave_deadline_time_for_store
    ...
    if _member_on_leave_prep_schedule_for_period(
        db,
        member,
        leave_meal_period=leave_meal_period,
...
def is_miniprogram_pause_delivery_prep_locked(
    ...
    *,
    ...
    affected = miniprogram_prep_lock_affected_delivery_date(now=n)
    if member_on_lunch_delivery_schedule(
        db, member, delivery_date=affected, today=n.date()
    ):
        return True
    return member_on_dinner_delivery_schedule(
        db, member, delivery_date=affected, today=n.date()
...
def guard_member_self_service_during_sf_fulfillment(
    ...
    *,
    ...
    from app.services.delivery.sf_order_fulfillment_service import member_sf_self_service_locked_on_delivery_date
