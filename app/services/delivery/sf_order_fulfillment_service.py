...
from sqlalchemy import Integer, String, and_, case, cast, exists, func, or_, select, text, true as sql_true
...
def _sf_push_lunch_sheet_clause():
    """午餐配送大表推单（含历史空 push_kind）。"""
    return or_(
...
def _sf_push_dinner_sheet_clause():
    return SfSameCityPush.push_kind == SF_PUSH_KIND_DINNER_DELIVERY_SHEET


def _sf_push_retail_order_id_expr():
    """零售单次 stop_id=retail-smo-{order_id} → 订单 id 表达式。"""
    return cast(func.replace(SfSameCityPush.stop_id, "retail-smo-", ""), Integer)


def _sf_push_single_meal_retail_meal_period_clause(meal_period: str):
    """单次零售推单按订单 meal_period 筛选。"""
    from app.models.enums import MealPeriod

    period = (meal_period or "").strip().lower()
    oid_expr = _sf_push_retail_order_id_expr()
    if period == MealPeriod.DINNER.value:
        period_match = SingleMealOrder.meal_period == MealPeriod.DINNER.value
    else:
        period_match = or_(
    ...
    return and_(
        SfSameCityPush.push_kind == SF_PUSH_KIND_SINGLE_MEAL_RETAIL,
        SfSameCityPush.stop_id.like("retail-smo-%"),
        exists(
            select(1)
            .select_from(SingleMealOrder)
            .where(
                SingleMealOrder.id == oid_expr,
                period_match,
            )
        ),
    )
...
def _sf_push_monitor_meal_period_clause(meal_period: str | None):
    """监控列表/统计：按午餐或晚餐餐段筛选（大表 push_kind + 单次零售订单 meal_period）。"""
    from app.models.enums import MealPeriod
    ...
    if period == MealPeriod.DINNER.value:
        return or_(
            _sf_push_dinner_sheet_clause(),
            _sf_push_single_meal_retail_meal_period_clause(MealPeriod.DINNER.value),
        )
    if period == MealPeriod.LUNCH.value:
        return or_(
            _sf_push_lunch_sheet_clause(),
            _sf_push_single_meal_retail_meal_period_clause(MealPeriod.LUNCH.value),
        )
    raise ValueError("meal_period 仅支持 lunch、dinner 或留空（全部）")
...
def _sf_push_monitor_where_clauses(
    *,
    ...
    mp_clause = _sf_push_monitor_meal_period_clause(meal_period)
    if mp_clause is not None:
        clauses.append(mp_clause)
...
def _apply_sf_same_city_stop_fulfillment(
    ...
    *,
    ...
    if home_ok_ids is None:
        from app.services.admin.admin_delivery_fulfillment_service import _eligible_ids_home
...
def bulk_admin_resync_subscription_fulfilled_from_sf_monitor_for_delivery_day(
    ...
    *,
    ...
    meal_period 可选 lunch/dinner，默认仅午餐大表（与历史行为一致）。
    """
    from app.models.enums import MealPeriod
    ...
    period = (meal_period or MealPeriod.LUNCH.value).strip().lower()
    if period == MealPeriod.DINNER.value:
        sheet_clause = _sf_push_dinner_sheet_clause()
        period_label = "晚餐"
    else:
        sheet_clause = _sf_push_lunch_sheet_clause()
        period_label = "午餐"
    rows = list(
        db.scalars(
    ...
    summary = (
        f"扫描 {scanned} 条{period_label}大表推单；订阅扣次 {subs_applied} 人次；"
...
def list_sf_same_city_pushes_for_monitor(
    ...
    *,
    ...
    agg_cache: dict[tuple[date, int, str], dict[str, Any]] = {}
    ...
    def _aggs_for_row(r: SfSameCityPush) -> dict[str, Any]:
        ...
        kind_raw = (getattr(r, "push_kind", None) or "").strip() or SF_PUSH_KIND_DELIVERY_SHEET
        period = _meal_period_from_push_kind(kind_raw)
        k = (d0, sid, period)
        if k not in agg_cache:
            agg_cache[k] = aggs_for_delivery_date(db, d0, store_id=sid, meal_period=period)
        return agg_cache[k]
