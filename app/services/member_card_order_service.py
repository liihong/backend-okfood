...
from app.models.enums import CardOrderKind, CardOrderPayStatus, PlanType
...
def _quota_for_card_kind(kind: str) -> tuple[PlanType, int]:
    if kind == CardOrderKind.WEEK.value:
        return PlanType.WEEK, 6
    if kind == CardOrderKind.MONTH.value:
...
def update_card_order(
    ...
    if not patch and not sync_flag:
