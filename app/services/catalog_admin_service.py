...
def list_retail_menu_public(db: Session, *, store_id: int) -> list[dict]:
    """按分类返回上架零售商品；仅含启用分类且至少一件上架商品。"""
    categories = list_retail_categories(db, store_id=store_id, active_only=True)
    products = list_retail_products(db, store_id=store_id, shelf_only=True)
    by_cat: dict[int, list[dict]] = {}
    for p in products:
        cid = int(p.category_id) if p.category_id is not None else None
        if cid is None:
            continue
        by_cat.setdefault(cid, []).append(retail_product_public_dump(p))
    out: list[dict] = []
    for c in categories:
        items = by_cat.get(int(c.id), [])
        if not items:
            continue
        out.append(
            {
...
def list_retail_catalog_public(db: Session, *, store_id: int) -> list[dict]:
    """小程序菜单页：启用分类 + 上架商品（按分类分组）。"""
    categories = list_retail_categories(db, store_id=int(store_id), active_only=True)
    products = list_retail_products(db, store_id=int(store_id), shelf_only=True)
    by_cat: dict[int, list[dict]] = {}
    for p in products:
        cid = int(p.category_id) if p.category_id is not None else None
        if cid is None:
