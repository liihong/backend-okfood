...
# 剩余为 0 时仍允许记损耗流水的原因（后厨/配送实损，remaining 展示仍钳制为 0）
_WASTE_ALLOWED_WHEN_NO_REMAINING = frozenset(
    {
        DayStockAdjustmentReason.SPILL.value,
        DayStockAdjustmentReason.KITCHEN_TASTE.value,
        DayStockAdjustmentReason.KITCHEN_WASTE.value,
        DayStockAdjustmentReason.COMP_MEAL.value,
        DayStockAdjustmentReason.OTHER.value,
    }
)
...
def create_day_stock_adjustment(
    ...
    *,
    ...
    if rem + d < 0:
        # 后厨/配送类实损：剩余为 0 时仍记流水，损耗统计增加，remaining 由公式 max(0,…) 保持为 0
        if d >= 0 or code not in _WASTE_ALLOWED_WHEN_NO_REMAINING:
            raise HTTPException(
                status_code=400,
                detail=f"调整后剩余将为 {rem + d} 份，不能小于 0（当前剩余 {rem} 份）",
            )
        new_waste = waste_total_display(before.adjustment_delta_sum + d)
        kitchen_cap = int(before.kitchen_output or 0)
        if new_waste > kitchen_cap:
            raise HTTPException(
                status_code=400,
                detail=f"累计损耗不能超过后厨产出量（{kitchen_cap} 份）",
            )
