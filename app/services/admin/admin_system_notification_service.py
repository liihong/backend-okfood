...
def _member_insufficient_balance_notification_exists(
    ...
    *,
    ...
    """同日同店同会员同餐段是否已有余量不足配送提醒（含已确认），避免轮询重复写入。"""
    period = (meal_period or "lunch").strip().lower()
    marker = f"mid={int(member_id)} period={period} "
    row_id = db.scalar(
...
def create_member_insufficient_balance_for_delivery_notification(
    ...
    *,
    ...
    """每日多份但剩余次数不足：提醒客服联系用户确认是否续卡（按餐段隔离）。"""
    sid = int(store_id)
    if sid <= 0:
        return None
    period = (meal_period or "lunch").strip().lower()
    ...
    if _member_insufficient_balance_notification_exists(
        db, store_id=sid, business_date=business_date, member_id=int(member_id), meal_period=period
    ):
        return None
    ...
    period_cn = "晚餐" if period == "dinner" else "午餐"
    title = f"续费跟进·余量不足配送({period_cn}) · {business_date.isoformat()}"
    body = (
        f"会员 mid={int(member_id)} period={period} {phones} "
        f"{('「' + naming + '」') if naming else ''}"
        f"供餐日 {business_date.isoformat()} {period_cn}应配送 {units} 份/日，剩余次数仅 {bal} 次，"
...
def scan_insufficient_balance_delivery_notifications_for_all_stores(
    ...
    *,
    ...
    from app.services.delivery.courier_service import list_members_insufficient_balance_for_delivery_day
    ...
    for sid in store_ids:
        sid_int = int(sid)
        for period in (MealPeriod.LUNCH.value, MealPeriod.DINNER.value):
            members = list_members_insufficient_balance_for_delivery_day(
                db, delivery_date=biz, store_id=sid_int, meal_period=period
            )
            for m in members:
                if period == MealPeriod.DINNER.value:
                    ...
                    bal, _ = dinner_balance_and_quota(row)
                    units = effective_daily_meal_units_for_period(
                        db, m, MealPeriod.DINNER.value
                    )
                else:
