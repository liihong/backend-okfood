...
class StoreRetailOrder(Base):
    __tablename__ = "store_retail_orders"
    __table_args__ = (UniqueConstraint("store_id", "out_trade_no", name="uk_sro_store_out_trade_no"),)
    ...
    retail_product_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("store_retail_products.id", onupdate="CASCADE"), index=True
    )
    ...
    fulfillment_date: Mapped[date] = mapped_column(Date, index=True)
