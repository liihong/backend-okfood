...
CREATE TABLE IF NOT EXISTS `member_addresses` (
  ...
  `map_location_text` VARCHAR(500) NULL COMMENT '收货位置主文案（与 door_detail 拼接为完整地址）',
  `door_detail` VARCHAR(500) NULL COMMENT '楼栋、单元、门牌等',
  `remarks` VARCHAR(500) NULL COMMENT '忌口/备注',
...
CREATE TABLE IF NOT EXISTS `member_card_orders` (
  ...
  `pay_channel` VARCHAR(10) NOT NULL COMMENT '缴费渠道：微信/支付宝/线下/抖音',
...
CREATE TABLE IF NOT EXISTS `home_entry_poster` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `store_id` BIGINT UNSIGNED NOT NULL,
  `poster_type` VARCHAR(32) NOT NULL DEFAULT 'entry' COMMENT '海报场景：entry=进入小程序 menu=菜单页',
  `image_url` TEXT NOT NULL COMMENT '海报图片 URL',
  `is_active` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否启用',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_home_entry_poster_store_type` (`store_id`, `poster_type`),
  CONSTRAINT `fk_home_entry_poster_store` FOREIGN KEY (`store_id`) REFERENCES `stores` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='小程序弹窗海报';
