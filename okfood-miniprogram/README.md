...
底部 **Tab**（会员）：「菜单」「订单」「我的」。Tab 图标放在项目根目录 **`static/`**（如 `order-nor.png` / `order-sel.png`），建议 **81×81 像素**、单张 **&lt; 40KB**；`pages.json` 与 `src/utils/customTabBar.js` 中路径需一致。uni-app 默认只打包「被引用」的 static 文件，自定义 tabBar 图标需靠 `vite.config.js` 的同步插件或 `node scripts/sync-static-to-mp-weixin.mjs` 写入 `dist/*/mp-weixin/static/`。
