...
from app.services.admin.day_stock_service import create_day_stock_adjustment, get_day_stock_breakdown
...
def _patch_stock_deps(monkeypatch, *, d: date, kitchen: int | None, home: int, pickup: int = 0, retail: int = 0):
    monkeypatch.setattr(
        "app.services.admin.day_stock_service.weekly_menu_day_total_stock",
        lambda db, menu_date, store_id, meal_period=MealPeriod.LUNCH.value: kitchen,
    )
    monkeypatch.setattr(
        "app.services.admin.day_stock_service._prep_metrics_for_period",
        lambda *a, **k: _fake_metrics(home=home, pickup=pickup),
    )
    monkeypatch.setattr(
        "app.services.admin.day_stock_service.paid_single_retail_portions_by_dates",
...
def test_waste_allowed_when_remaining_zero_for_kitchen_reason(db: Session, monkeypatch):
    """剩余为 0 时仍允许后厨报损（仅增损耗统计，remaining 保持 0）。"""
    d = date(2026, 7, 1)
    _patch_stock_deps(monkeypatch, d=d, kitchen=240, home=220, pickup=1, retail=19)
    ...
    after = create_day_stock_adjustment(
        ...
        meal_period=MealPeriod.LUNCH.value,
        delta=-2,
        reason_code=DayStockAdjustmentReason.KITCHEN_WASTE.value,
        remark=None,
        operator="test_admin",
    )
...
def test_lunch_dinner_kitchen_independent(db: Session, monkeypatch):
    ...
    monkeypatch.setattr("app.services.admin.day_stock_service.weekly_menu_day_total_stock", _kitchen)
    ...
    monkeypatch.setattr("app.services.admin.day_stock_service._prep_metrics_for_period", _metrics)
    monkeypatch.setattr(
        "app.services.admin.day_stock_service.paid_single_retail_portions_by_dates",
        lambda *a, **k: {d: 0},
    )
