<script setup>
defineOptions({ name: 'MenuPagePosterView' })
...
async function loadConfig() {
  ...
  try {
    const data = await apiJson(
      `/api/admin/marketing/menu-poster?store_id=${storeId.value}`,
      {},
      { auth: true },
    )
...
<template>
  <div v-loading="loading" class="menu-poster-page">
    <div class="page-card">
      <div class="page-card__head">
        <h2 class="page-card__title">菜单页弹窗海报</h2>
        <p class="page-card__desc">
          用户每次进入点单菜单页时弹出，可用于宣传新菜品、果蔬汁等上新内容。建议上传竖版长图，内容简洁醒目。
        </p>
      </div>
