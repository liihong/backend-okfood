<script setup>
/**
 * 门店坐标：高德地图搜索选点 + 点击地图 + 拖拽标记（GCJ-02，与后端一致）。
 */
...
function storePickPinIconDataUrl() {
  const svg =
...
const emit = defineEmits(['update:lngStr', 'update:latStr', 'warn'])
...
/** 与高德 AutoComplete 绑定的 input 须稳定 id */
const searchInputId = 'store-config-amap-poi-search'
...
async function initMap() {
  ...
  await new Promise((resolve) => {
    AMap.plugin(['AMap.PlaceSearch', 'AMap.AutoComplete'], () => resolve())
  })
...
function finishSearchPick(lng, lat) {
...
<template>
  <div class="slp">
    <p v-if="!amapKey" class="slp-warn">
      未配置 <code>VITE_AMAP_KEY</code>，无法使用地图选点；请仍通过其它地图拾取 GCJ-02 坐标后手动填写下方经纬度。
    </p>
    <template v-else>
      <div class="slp-search">
        ...
        />
        <button type="button" class="slp-search-btn" @click="runKeywordSearch">搜索</button>
      </div>
      <p class="slp-hint">
        在输入框选用联想结果，或点击「搜索」取第一条 POI；也可<strong>直接点击地图</strong>、<strong>拖动红色标记</strong>微调。坐标系为
        GCJ-02（高德）。
      </p>
      <div ref="mapEl" class="slp-map" />
    </template>
  </div>
</template>
